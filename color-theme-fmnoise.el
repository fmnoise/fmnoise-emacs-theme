;;; color-theme-fmnoise.el

(require 'color)

(defun fmnoise--interpolate (hex1 hex2 gradations which)
  (let ((c1 (color-name-to-rgb hex1))
        (c2 (color-name-to-rgb hex2)))
    (apply 'color-rgb-to-hex (nth which (color-gradient c1 c2 gradations)))))

(defun fmnoise--alt-background (background highlight)
...
executed."
  (fmnoise--interpolate background highlight 7 3))

(defconst color-theme-fmnoise-colors
...
(defmacro color-theme-fmnoise--face-specs ()
  ...
  (quote
   (mapcar
    ...
    `(;; Standard font lock faces
      ...
      (diff-removed (:foreground, red)) ;; orange
      ...
      (company-tooltip-common (:inherit company-tooltip :foreground ,orange)) ;; red
      (company-tooltip-common-selection (:inherit company-tooltip-selection :foreground ,orange)) ;; red
      ...
      (diredfl-exec-priv (:foreground ,blue :background nil))
      ...
      (magit-diff-removed-highlight (:foreground ,red :background ,highlight))
...
(defmacro color-theme-fmnoise--frame-parameter-specs ()
...
(defun color-theme-fmnoise--theme-name (mode)
...
(defmacro color-theme-fmnoise--define-theme (mode)
  ...
  (let ((name (color-theme-fmnoise--theme-name mode))
    ...
    `(progn
       ...
       (color-theme-fmnoise--with-colors
        ',mode
        (apply 'custom-theme-set-faces ',name
               (color-theme-fmnoise--face-specs))
  ...
  (if (fboundp 'load-theme)
    ...
    (progn
      ...
      (color-theme-fmnoise--with-colors
       mode
       (color-theme-install
        `(,(intern (concat "color-theme-fmnoise-" (symbol-name mode)))
          ,@(color-theme-fmnoise--frame-parameter-specs)
          ,@(color-theme-fmnoise--face-specs)))
...
;;; color-theme-fmnoise.el ends here
